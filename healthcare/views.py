        ...
        if request.method == "POST":
            try:
                ...
                registrationNumber = (
                    name.replace(" ", "")
                    + uuidNo
                    + str(random.randint(2345678909800, 9923456789000))[0:5]
                )
                patientData = Patient.objects.create(
                    name=name,
                    mobile=mobile,
                    email=email,
                    gender=gender,
                ...
                return render(
                    request, "newPatient.html", {"message": "Something went Wrong"}
                )
            try:
                send_mail(
                ...
                print(e)
                return render(
                    request, "newPatient.html", {"message": "Failed To Send Email"}
                )
            finally:
                return render(
                    request,
                    "newPatient.html",
                    {"success": True, "patientId": patientData.id},
                )
        else:
...
def updatePatientRecord(request, patientId):
    try:
        ...
        if len(patient) == 0:
            return render(
                request, "patientRecord.html", {"message": "Patient desnot exist!"}
            )
        ...
        if request.method == "POST":
            ...
            return render(
                request,
                "updatePatientRecord.html",
                {"patient": patientRecord, "success": True, "profile": patientP},
            )
            ...
            return render(
                request,
                "updatePatientRecord.html",
                {"patient": patientRecord, "profile": patientP},
            )
    except Exception as e:
        print(e)
        return HttpResponse("<h1>something went wrong!!!</h1>")
