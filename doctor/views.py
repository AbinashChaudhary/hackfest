...
def medicineFile(request, prescriptionId):
    ...
    medicinDirMap = MedicineDirPrescriptionMap.objects.filter(
    ...
    if len(medicinDirMap) != 0:
        ...
        for entry in medicinDirMap:
            medsDir = MedicineDirection.objects.filter(
                pk=entry.medicineDirectionId.id
            ).first()
            medsName = Medicine.objects.filter(pk=medsDir.medicineId.id).first()
            medsDirList.append({"medsDir": medsDir, "medsName": medsName})
        lines = []
...
def searchPatient(request):
    try:
        ...
        if request.session["role"] != "Doctor" and request.session["role"] != "Nurse":
            return render(
                request, "index.html", {"messages": "You Are Not Authenticated"}
            )
    ...
    try:
        ...
        if len(his) != 0:
            ...
            return render(
                request,
                "viewPatientRecord.html",
                {"noRecord": True, "details": patientDetails},
            )
    except Exception as e:
        print(e)
        messages.add_message(request, messages.ERROR, "Please Add Valid Details !")
...
def viewMedicine(request, medicineId, prescriptionId):
    try:
        ...
        return render(
            request,
            "viewMedicine.html",
            {"medicineDetails": medicineDetails, "prescriptionId": prescriptionId},
        )
    except Exception as e:
        print(e)
...
def viewPrescription(request, prescriptionId):
    try:
        ...
        laboratoryTest = LabTestPrescriptionMap.objects.filter(
            prescriptionId=prescription
        )
        tests = []
    ...
    try:
        if request.method == "POST":
            ...
            with transaction.atomic():
                laboratoryTestData = LaboratoryTest.objects.create(
                    testName=testName,
                    testBodySite=testBodySite,
                    testUse=testUse,
                    testDescription=testDescription,
                    testSpecimen=testSpecimen,
                )
                prescription = Prescription.objects.get(pk=prescriptionId)
                labTestData = LabTestPrescriptionMap.objects.create(
                    laboratoryTestId=laboratoryTestData, prescriptionId=prescription
                )
        ...
        return render(
            request,
            "labTest.html",
            {"prescriptionId": prescriptionId, "message": "Something Went Wrong!"},
        )
...
def diagnosis(request, patientId):
    try:
        ...
        if request.method == "POST":
            ...
            with transaction.atomic():
                diagnosisData = Diagnosis.objects.create(
                    diagnosisName=diagnosisName,
                    diagnosisBodySite=diagnosisBodySite,
                    dateOfOnset=dateOfOnset,
                    severity=severity,
                    dateOfAbatement=dateOfAbatement,
                    diagnosisCertainity=diagnosisCertainity,
                    diagnosisDescription=diagnosisDescription,
                )
                deviceData = MedicalDevice.objects.create(
                    deviceName=deviceName,
                    deviceBodySite=deviceBodySite,
                    deviceUse=deviceUse,
                    deviceDscription=deviceDescription,
                )
                prescriptionData = Prescription.objects.create(
                    patientId=patient,
        ...
        return render(
            request,
            "diagnosisPage.html",
            {"patient": patient, "message": "Something Went Wrong!"},
        )
...
def medication(request, prescriptionId):
    try:
        ...
        if request.method == "POST":
            ...
            with transaction.atomic():
                medicationData = MedicineDirection.objects.create(
                    medicineId=medicine,
                    doseUnit=doseUnit,
                    duration=duration,
                ...
                prescription = Prescription.objects.get(pk=prescriptionId)
                medicationDirData = MedicineDirPrescriptionMap.objects.create(
                    prescriptionId=prescription, medicineDirectionId=medicationData
                )
            return render(
                request,
                "medicationPage.html",
                {
                    "prescriptionId": prescriptionId,
                    "allMeds": allMeds,
                    "success": "Medicine Added Successfullty",
                },
            )
        else:
            return render(
                request,
                "medicationPage.html",
                {"prescriptionId": prescriptionId, "allMeds": allMeds,},
            )
    except Exception as e:
        print(e)
        allMeds = Medicine.objects.all()
